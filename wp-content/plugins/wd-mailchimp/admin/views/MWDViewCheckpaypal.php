...
class MWDViewCheckpaypal {
  ...
  public function display() {
    ...
    $tax = $form->tax;
    $shipping = floatval($_POST['mc_shipping']);
    ...
    if (!$row) {
      $wpdb->insert($wpdb->prefix . "mwd_forms_sessions", array(
        'form_id' => $id,
        'group_id' => $group_id,
      ...
      $wpdb->update($wpdb->prefix . "mwd_forms_sessions", array(
        'form_id' => $id,
        'full_name' => $_POST['first_name'] . " " . $_POST['last_name'],
        'email' => $_POST['payer_email'],
        'phone' => $_POST['night_ phone_a'] . " - " . $_POST['night_ phone_b'] . " - " . $_POST['night_ phone_c'],
    ...
    if ($form->mail) {
      ...
      $send = wp_mail($recipient, $subject, stripslashes($body), $header_arr, '');
    }
    return 0;
  }
