...
class MWDViewWidget {
	...
	function widget($args, $instance) {
		...
		$controller_class = 'MWDControllerForms';
		$controller = new $controller_class();
		echo $controller->execute($instance['form_id'], 'embedded');
		// After widget.
		echo $after_widget;
	}
	...
	function form($instance, $id_title, $name_title, $id_form_id, $name_form_id) {
		...
		<p>
			...
			<select name="<?php echo $name_form_id; ?>" id="<?php echo $id_form_id; ?>" style="width:225px;text-align:center;">
				...
				<?php 
				$ids_Form_Maker = $this->model->get_forms();
				foreach ($ids_Form_Maker as $arr_Form_Maker) { ?>
					<option value="<?php echo $arr_Form_Maker->id; ?>" 
					<?php if ($arr_Form_Maker->id == $instance['form_id']) {
						echo "SELECTED";
					} ?>>
						<?php echo $arr_Form_Maker->title; ?>
					</option>
				<?php } ?>
			</select>
		</p>
