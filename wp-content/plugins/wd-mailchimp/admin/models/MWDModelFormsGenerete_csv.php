...
class MWDModelFormsGenerete_csv {
	////////////////////////////////////////////////////////////////////////////////////////
	// Events                                                                             //
	////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////////////////////
	...
	public function get_data() {
		...
		$query = $wpdb->prepare("SELECT distinct element_label FROM " . $wpdb->prefix . "mwd_forms_submits where form_id=%d",$form_id);	
		$labels = $wpdb->get_col($query);
		
		$query_lable = $wpdb->prepare("SELECT label_order,title FROM " . $wpdb->prefix . "mwd_forms where id=%d", $form_id);
		...
		$query = $wpdb->prepare("SELECT group_id, ip, date, user_id_wd, GROUP_CONCAT( element_label SEPARATOR ',') as element_label, GROUP_CONCAT( element_value SEPARATOR '*:*el_value*:*') as element_value FROM " . $wpdb->prefix . "mwd_forms_submits where form_id= %d GROUP BY group_id ORDER BY date ASC limit %d, %d", $form_id, $limitstart, 1000);
		...
		for ($www = $limitstart; $www < $group_id_s_count; $www++) {
			...
			if($label_type[$field_key] != 'type_arithmetic_captcha') {
				...
				$query = $wpdb->prepare("SELECT id FROM " . $wpdb->prefix . "mwd_forms_submits where element_label=%s AND form_id = %d AND group_id=%d",'item_total', $form_id, $i);	
				$is_paypal = $wpdb->get_results($query);
			
				if($is_paypal) {
					$item_total = $wpdb->get_var($wpdb->prepare("SELECT `element_value` FROM " . $wpdb->prefix . "mwd_forms_submits where group_id=%d AND element_label=%s", $i, 'item_total'));        
					$total = $wpdb->get_var($wpdb->prepare("SELECT `element_value` FROM " . $wpdb->prefix . "mwd_forms_submits where group_id=%d AND element_label=%s", $i, 'total'));        
					$payment_status = $wpdb->get_var($wpdb->prepare("SELECT `element_value` FROM " . $wpdb->prefix . "mwd_forms_submits where group_id=%d AND element_label=%s", $i, '0'));        
					$data_temp['Item Total'] = $item_total;
					$data_temp['Total'] = $total;
					$data_temp['Payment Status'] = $payment_status;
				}
				$query = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "mwd_forms_sessions where group_id=%d",$i);
